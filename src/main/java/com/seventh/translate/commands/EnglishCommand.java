...
@Component
public class EnglishCommand implements MessageCommand {
    @Autowired
    ...
    private InteractionFollowupCreateMono handleTranslation(MessageInteractionEvent event) throws URISyntaxException, IOException, InterruptedException {
        ...
        if (detectedLanguage.isBlank()) {
            return event
                    .createFollowup("ERROR: This language is not supported")
                    .withEphemeral(true);
        }
        String langPair = detectedLanguage + "|en";
        if (langPair.equals("en|en")) {
            isSameLang = true;
        }

        Optional<Instant> editedTimestamp = resolvedMessage.getEditedTimestamp();
        String cache_message_id = guild_identifier + "_" + channel_id + "_" + message_id + "_en";
        ...
        return event
                ...
                .withEmbeds(
                    EmbedCreateSpec
                            ...
                            .title(EmbedTitleEnum.EN.langTitle)
